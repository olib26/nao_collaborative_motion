...
//====> ici j'ai rajouté les NAO_ID pour spécifier le nom
	...
	void receiveImage(const sensor_msgs::ImageConstPtr& msg) {
		...
		//EXAMPLES:
		//test7(imgcam);
		//publishImage(test5(imgcam),WINDOW);
	...
	Mat test3(Mat img_cam){
		Mat grad = getContourBall(img_cam,150,180,80,255,80,255);
		return grad;
	}
	//////////
...
//====>LA
	...
	bool ballPosForHand(nao_behavior_tree::BallPosForHand::Request &req,
		...
		publishImage(drawMatFromList (chain,imgcam),WINDOW2);
		publishImage(color,WINDOW3);
